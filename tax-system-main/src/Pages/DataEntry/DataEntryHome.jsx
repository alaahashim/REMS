...
import {
  Card, Row, Col, Table, Badge, Button, Container, Form, Nav, Spinner, Tab
...
import {
  getProperties, deleteProperty, getPropertiesWithUnits, deleteUnit
} from '../../services/propertyService';
import {
  getExemptionsForHome, deleteExemption
...
const DataEntryHome = () => {
  ...
  const handleSelectRequestSuggestion = (suggestion) => {
    setRequestSearch(suggestion.label);
    setSelectedRequest(suggestion.label);
  ...
  const handleSelectOwner = (owner) => {
    setShowOwnerDropdown(false);
    navigate(`/data-entry/owner/${owner.id}`);
  };
  ...
  const handleDeleteRequest = async (id) => {
    ...
    try {
      ...
      if (!target) return;
      if (target.type === 'طعن') await deleteAppeal(id);
      else if (target.type === 'إعفاء') await deleteExemption(id);
  ...
  const handleDeleteProperty = async (id) => {
    ...
    try {
      ...
      setPropertiesWithUnits(prev => prev.filter(p => p.id !== id));
          ...
          <Tab.Container id="main-tabs" defaultActiveKey="pending">
            ...
            <Tab.Content>
              ...
              <Tab.Pane eventKey="pending">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div ref={reqSearchRef} style={{ width: '300px', position: 'relative' }}>
                    ...
                    {showReqDropdown && (
                      <div className="border rounded bg-white shadow-sm" style={{ position: 'absolute', top: '100%', right: 0, left: 0, zIndex: 1050, maxHeight: '220px', overflowY: 'auto' }}>
                        {requestSuggestions.map((s, i) => (
                          <div key={i} className="px-3 py-2 d-flex align-items-center gap-2" style={{ cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.background = '#f0f4ff')} onMouseLeave={e => (e.currentTarget.style.background = '')} onMouseDown={() => handleSelectRequestSuggestion(s)}>
                            <i className="fa-solid fa-user text-primary small"></i>
                            <span style={{ fontSize: '0.9rem' }}>{s.label}</span>
                            {s.nationalId && (<span className="text-muted font-monospace ms-auto" style={{ fontSize: '0.78rem' }}>{s.nationalId}</span>)}
                          </div>
                        ))}
                      </div>
                    )}
                ...
                <Table hover responsive className="align-middle">
                  <thead className="table-light"><tr><th>النوع</th><th>كود / رقم الوحدة</th><th>اسم المالك</th><th>تاريخ الطلب</th><th>السند القانوني</th><th>الحالة</th><th className="text-end pe-4">إجراءات</th></tr></thead>
                  <tbody>
                        ...
                        <tr key={`${req.type}-${req.id}`}>
                          ...
                          <td className="text-end pe-4">
                            <div className="d-flex justify-content-end gap-1">
                              <Button variant="light" size="sm" className="text-primary border" onClick={() => handleEditRequest(req)}><i className="fa-solid fa-pen-to-square"></i></Button>
                              <Button variant="light" size="sm" className="text-danger border" onClick={() => handleDeleteRequest(req.id)}><i className="fa-solid fa-trash"></i></Button>
                            </div>
                          </td>
                        </tr>
                      ))
              ...
              <Tab.Pane eventKey="decided">
                <Table hover responsive className="align-middle">
                  <thead className="table-light"><tr><th>النوع</th><th>الرقم القومي</th><th>اسم المالك</th><th>تاريخ الطلب</th><th>السند القانوني</th><th>الحالة النهائية</th></tr></thead>
                  <tbody>
                    {loading ? (<tr><td colSpan="6" className="text-center py-5"><Spinner animation="border" /></td></tr>) : 
              ...
              <Tab.Pane eventKey="owners">
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div ref={ownerSearchRef} style={{ width: '300px', position: 'relative' }}>
                    ...
                    {showOwnerDropdown && (
                      <div className="border rounded bg-white shadow-sm" style={{ position: 'absolute', top: '100%', right: 0, left: 0, zIndex: 1050, maxHeight: '260px', overflowY: 'auto' }}>
                        {ownerSuggestions.map(owner => (
                          <div key={owner.id} className="px-3 py-2 d-flex align-items-center gap-2" style={{ cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.background = '#f0f4ff')} onMouseLeave={e => (e.currentTarget.style.background = '')} onMouseDown={() => handleSelectOwner(owner)}>
                            <div className="bg-primary bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: 30, height: 30 }}><i className="fa-solid fa-user text-primary" style={{ fontSize: '0.75rem' }}></i></div>
                            <div className="flex-grow-1 overflow-hidden">
                              <div className="fw-semibold text-truncate" style={{ fontSize: '0.88rem' }}><DynText text={owner.fullName} lang={lang} /></div>
                              <div className="text-muted font-monospace" style={{ fontSize: '0.75rem' }}>{owner.nationalId}</div>
                            </div>
              ...
              <Tab.Pane eventKey="properties">
                    ...
                    {showUnitDropdown && (
                      <div className="border rounded bg-white shadow-sm" style={{ position: 'absolute', top: '100%', right: 0, left: 0, zIndex: 1050, maxHeight: '260px', overflowY: 'auto' }}>
                        {unitSuggestions.map(unit => (
                          <div key={unit.id} className="px-3 py-2 d-flex align-items-center gap-2" style={{ cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.background = '#f0f4ff')} onMouseLeave={e => (e.currentTarget.style.background = '')} onMouseDown={() => handleSelectUnit(unit)}>
                            <div className="bg-success bg-opacity-10 rounded-circle d-flex align-items-center justify-content-center flex-shrink-0" style={{ width: 30, height: 30 }}><i className="fa-solid fa-house text-success" style={{ fontSize: '0.75rem' }}></i></div>
                            <div className="flex-grow-1 overflow-hidden">
                              <div className="fw-semibold text-truncate" style={{ fontSize: '0.88rem' }}>الوحدة: {unit.unitNumber || '-'}</div>
                              <div className="text-muted small text-truncate">العقار #{unit.propertyId} — الدور {unit.floor}</div>
                            </div>
                            <i className="fa-solid fa-arrow-left text-muted small"></i>
                          </div>
                        ))}
                ...
                <Table hover responsive className="align-middle">
                  <thead className="table-light"><tr><th>رقم العقار</th><th>رقم المبنى</th><th>كود الوحدة</th><th>نوع الوحدة</th><th>الدور</th><th>المساحة</th><th>الاستخدام</th><th>التشطيب</th><th>الحالة</th><th className="text-end pe-4">إجراءات</th></tr></thead>
                  <tbody>
                   {loading ? (<tr><td colSpan={10} className="text-center py-5"><Spinner animation="border" /></td></tr>) :  
                    getDisplayedUnits().length > 0 ? (
                      getDisplayedUnits().map(unit => (
                        <tr key={unit.id}>
                          ...
                          <td className="text-end pe-4">
                            <Button variant="light" size="sm" className="text-danger border" onClick={() => handleDeleteUnit(unit.id)}><i className="fa-solid fa-trash"></i></Button>
                          </td>
                        </tr>
                      ))
