...
const EditProperty = () => {
  ...
  // 2. قائمة الملاك (جديدة - مهمة جداً)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (prop.centerId) {
          const loadedNeighborhoods = await getNeighborhoods(prop.centerId);
  ...
  // تحميل المراكز والشوارع والاحياء (Cascade)
  useEffect(() => {
    if (propertyData.governorateId) {
      getCenters(Number(propertyData.governorateId)).then(data => { setCenters(data); setStreets([]); setNeighborhoods([]); });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log("Saving Owners:", ownersList);
      
      setTimeout(() => {
        setMessage({ text: 'تم تحديث البيانات وقائمة الملاك بنجاح!', type: 'success' });
        setSaving(false);
            ...
            <Card.Body>
              ...
              {loading ? (
                ...
                <Form onSubmit={handleSubmit}>
                  ...
                  {/* القسم الثاني: جدول الملاك (الميزة الاحترافية) */}
