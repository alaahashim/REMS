...
// لترجمة قوائم المحافظات والمراكز والشوارع
...
// لترجمة اسم المالك ووضعه في الـ Input (بدون ما نكسر قواعد الـ Hooks)
const TranslatedOwnerInput = ({ value, lang }) => {
...
const EditProperty = () => {
  ...
  const [propertyData, setPropertyData] = useState({
    governorateId: '', centerId: '', neighborhoodId: '', streetId: '', buildingNo: '', description: ''
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const govs = await getGovernorates(); setGovernorates(govs);
        const prop = await getPropertyById(id);
        if (!prop) throw new Error('العقار غير موجود');

        setPropertyData({
          governorateId: prop.governorateId || '', centerId: prop.centerId || '',
          neighborhoodId: prop.neighborhoodId || '', streetId: prop.streetId || '',
          buildingNo: prop.buildingNo || '', description: prop.description || ''
        });

        const allAssignments = await getAssignments();
        setOwnersList(allAssignments.filter(a => String(a.propertyId) === String(id)).map(a => ({
          id: a.id, personId: a.personId, personName: a.name,
          shareType: a.shareType || 'Owner', sharePercentage: a.sharePercentage || 100, roleType: a.roleType || 'Owner'
        })));
        setLoading(false);
      } catch (error) {
        setMessage({ text: error.message || 'فشل تحميل البيانات', type: 'danger' }); setLoading(false);
      }
    };
    fetchData();
  }, [id]);
  ...
  useEffect(() => {
    if (propertyData.governorateId) getCenters(Number(propertyData.governorateId)).then(data => setCenters(data));
  ...
  const addOwnerRow = () => {
    setOwnersList([...ownersList, { id: Date.now(), personId: '', personName: '', shareType: 'Sale', sharePercentage: 0, roleType: 'Owner' }]);
  };
  ...
  const handleOwnerChange = (index, field, value) => {
    const newOwners = [...ownersList]; newOwners[index][field] = value; setOwnersList(newOwners);
  };

  const openCitizenSearch = (index) => { setActiveOwnerIndex(index); setShowCitizenModal(true); };

  const handleSelectCitizen = (citizen) => {
    if (activeOwnerIndex === null) return;
    setOwnersList(prev => {
      const next = [...prev];
      next[activeOwnerIndex] = { ...next[activeOwnerIndex], personId: citizen.nationalId || '', personName: citizen.name || '' };
      return next;
    });
    setShowCitizenModal(false); setActiveOwnerIndex(null);
  };

  const handlePropertyChange = (e) => {
    const { name, value } = e.target;
    if (name === 'governorateId') setPropertyData({ ...propertyData, [name]: value, centerId: '', neighborhoodId: '', streetId: '' });
    else if (name === 'centerId') setPropertyData({ ...propertyData, [name]: value, neighborhoodId: '', streetId: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const dto = {
        governorateId: Number(propertyData.governorateId), centerId: Number(propertyData.centerId),
        neighborhoodId: Number(propertyData.neighborhoodId), streetId: Number(propertyData.streetId),
        buildingNo: propertyData.buildingNo, description: propertyData.description,
      };
                    ...
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Select name="governorateId" value={propertyData.governorateId} onChange={handlePropertyChange} required>
                          ...
                          {governorates.map(g => <TranslatedOption key={g.id} id={g.id} name={g.name} lang={lang} />)}
                        </Form.Select>
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Select name="centerId" value={propertyData.centerId} onChange={handlePropertyChange} required disabled={!propertyData.governorateId}>
                          ...
                          {centers.map(c => <TranslatedOption key={c.id} id={c.id} name={c.name} lang={lang} />)}
                        </Form.Select>
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Select name="neighborhoodId" value={propertyData.neighborhoodId} onChange={handlePropertyChange} required disabled={!propertyData.centerId}>
                          ...
                          {neighborhoods.map(n => <TranslatedOption key={n.id} id={n.id} name={n.name} zone={n.zone} lang={lang} />)}
                        </Form.Select>
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Select name="streetId" value={propertyData.streetId} onChange={handlePropertyChange} required disabled={!propertyData.centerId}>
                          ...
                          {streets.map(s => <TranslatedOption key={s.id} id={s.id} name={s.name} lang={lang} />)}
                        </Form.Select>
                      </Form.Group>
                    </Col>
                  </Row>
                      ...
                      <tbody>
                        {ownersList.map((owner, index) => (
                          <tr key={owner.id}>
                            <td>
                              <InputGroup size="sm">
                                {/* 5. ترجمة اسم المالك الجاي من الداتا بيز */}
                                <TranslatedOwnerInput value={owner.personName} lang={lang} />
                                <Button variant="outline-primary" size="sm" type="button" onClick={() => openCitizenSearch(index)}>
                                  <i className="fa-solid fa-search"></i>
                                </Button>
                              </InputGroup>
                            </td>
                            <td>
                              <Form.Control type="number" size="sm" value={owner.sharePercentage} onChange={(e) => handleOwnerChange(index, 'sharePercentage', e.target.value)} required min="0" max="100" />
                            </td>
                            <td>
                              <Form.Select size="sm" value={owner.shareType} onChange={(e) => handleOwnerChange(index, 'shareType', e.target.value)}>
