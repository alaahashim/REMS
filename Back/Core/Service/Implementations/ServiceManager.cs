...
namespace Core.Service.Implementations
{
    public class ServiceManager : IServiceManager
    {
            ...
            INotificationService notificationService)
        {
            ...
            _employeeService = new(() => new EmployeeService(unitOfWork, httpContextAccessor));
            _auditLogService = new(() => new AuditLogService(unitOfWork));
            _identityService = new(() => new IdentityService(unitOfWork, notificationService));
            _lazyExemptionService = new(() => new ExemptionService(unitOfWork, mapper));
            _lazyTaxAssessmentService = new(() => new TaxAssessmentService(unitOfWork, mapper, this));
