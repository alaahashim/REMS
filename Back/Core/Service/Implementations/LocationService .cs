...
{
    var spec =
        new NeighborhoodByCenterSpecification(centerId);

    var repo =
        _unitOfWork.GetRepository<Neighborhood, int>();

    var data =
        await repo.GetAllAsync(spec);

    return data.Select(n => new NeighborhoodDto
    {
        Id = n.Id,
        CenterId = n.CenterId,
        Name = n.Name,
